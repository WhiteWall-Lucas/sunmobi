...
export const fetchTodayChargesLogic = async (authorization: string): Promise<FilteredCustomer[]> => {
                    ...
                    if ((paymentMethodId === 49489 || public_name === 'Boleto bancário') && mobilePhone !== undefined) {
                        ...
                        const hasSpecificTemplate = history.items.some(
                            (item: Item) =>
                                ...
                                item.content.template?.name === message.dia_da_emissao_da_fatura.templateName,
                        )
